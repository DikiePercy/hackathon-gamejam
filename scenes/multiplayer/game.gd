...
func _ready() -> void:
	...
	if _back_button:
		_back_button.connect("pressed", Callable(self, "_on_back_pressed"))
...
func _on_player_shot(origin: Vector2, direction: Vector2) -> void:
	...
	rpc("rpc_spawn_bullet", _local_peer_id, origin, direction)
...
func _spawn_bullet(owner_id: int, origin: Vector2, direction: Vector2) -> void:
	...
	bullet.body_entered.connect(Callable(self, "_on_bullet_body_entered").bind(owner_id))
	add_child(bullet)
...
func _physics_process(_delta: float) -> void:
	if _local_player == null or _remote_player == null:
		return
	if _local_player.has_method("get_network_state"):
		var state = _local_player.get_network_state()
		rpc("rpc_update_player_state", _local_peer_id, state.position, state.facing_right, state.aim_direction)
